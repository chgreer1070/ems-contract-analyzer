...
- `npm run db:migrate` performs a read-only canonical-receipt target preflight before Better Auth can change its schema, then rechecks under the ContractTwin advisory lock inside each migration phase. Do not bypass that orchestration by invoking a library migration API directly.
