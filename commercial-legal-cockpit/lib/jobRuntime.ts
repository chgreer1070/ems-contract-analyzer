...
export async function getJob(jobId:string){
  const result=await query<ProcessingJob & {next_attempt_at:Date}>(`select id,matter_id,document_id,job_type,status,attempts,max_attempts,input,output,external_operation_url,next_attempt_at from processing_jobs where id=$1 limit 1`,[jobId]);
  return result.rows[0]??null;
}

export async function claimJob(jobId:string,workerId:string){
  return withTransaction(async client=>{
    const result=await client.query<ProcessingJob & {next_attempt_at:Date}>(`select id,matter_id,document_id,job_type,status,attempts,max_attempts,input,output,external_operation_url,next_attempt_at from processing_jobs where id=$1 for update`,[jobId]);
    const job=result.rows[0];if(!job)return null;
    if(job.status==="SUCCEEDED"||job.status==="FAILED"||job.status==="CANCELLED")return job;
    if(job.status==="RUNNING"&&job.locked_by!==workerId)return null;
