...
import rawManifest from "@/lib/critical-database-controls.json";

type TriggerControl = { table: string; name: string; function: string };
type ColumnControl = { table: string; name: string; type: string; nullable: boolean };
type FunctionControl = { name: string; identityArguments: string; result: string; volatility: string };
type ConstraintControl = { table: string; name: string; type: string; validated: boolean };
type IndexControl = { table: string; name: string; unique: boolean };
type RuleControl = { table: string; name: string };

export type CriticalDatabaseControlManifest = {
  version: number;
  ...
  constraints: ConstraintControl[];
  indexes: IndexControl[];
};

export type CriticalDatabaseControlRow = {
  kind: "trigger" | "function" | "constraint" | "index" | "column" | "rule" | "server";
...
export const criticalDatabaseControlManifest = rawManifest as CriticalDatabaseControlManifest;
...
function tableObjectKey(table: string | null, name: string) {
  return `${table ?? ""}\u0000${name}`;
}

function objectMaps(rows: CriticalDatabaseControlRow[]) {
  return {
...
export function evaluateCriticalDatabaseControls(
  rows: CriticalDatabaseControlRow[],
  manifest: CriticalDatabaseControlManifest = criticalDatabaseControlManifest
) {
  const errors: string[] = [];
  ...
  return {
    ok: errors.length === 0,
    errors,
    checkedCount: manifest.columns.length + manifest.rules.length + manifest.triggers.length + manifest.functions.length + manifest.constraints.length + manifest.indexes.length,
