...
async function processMalwareScan(job:ProcessingJob){
  ...
  if(doc.security_scan_status==="CLEAN"){await completeJob(job.id,{securityScanStatus:"CLEAN",alreadyScanned:true});return;}
  ...
  try{
    ...
    await withTransaction(async client=>{
      ...
      await client.query("update processing_jobs set status=$2,output=$3::jsonb,error_message=$4,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,result.clean?"SUCCEEDED":"FAILED",JSON.stringify({securityScanStatus:status}),result.clean?null:"Malware scanner quarantined the source document."]);
    });
  }catch(error){
...
async function persistChunks(documentId:string,matterId:string,chunks:Array<{pageNumber:number|null;chunkIndex:number;text:string;sha256:string}>,method:string,pageCount:number|null,serverSha:string,expectedExtractionJobId:string,actor:{id:string;name:string}){
  await withTransaction(async client=>{
...
async function processExtract(job:ProcessingJob){
  ...
  const doc=await withTransaction(async client=>{
    const running=(await client.query<{matter_id:string|null;document_id:string|null;job_type:string;status:string}>("select matter_id,document_id,job_type,status from processing_jobs where id=$1 for update",[job.id])).rows[0];
    if(!running||running.status!=="RUNNING"||running.job_type!=="EXTRACT"||running.document_id!==job.document_id)throw new TerminalJobError("Extraction generation is not an exact RUNNING EXTRACT job.");
    ...
    if(running.matter_id!==current.matter_id)throw new TerminalJobError("Extraction job matter lineage is invalid.");
  ...
  if(!extraction.chunks.length){
    ...
    const ocrJob=await enqueueJob({matterId:doc.matter_id,documentId:doc.id,jobType:"OCR",idempotencyKey:`ocr:${job.id}:${serverSha}`,createdBy:String(job.input?.requestedBy||"system-worker"),input:{...job.input,extractionJobId:job.id},maxAttempts:3});
    await completeJob(job.id,{delegatedTo:"OCR",ocrJobId:ocrJob.id,serverSha256:serverSha,warnings:extraction.warnings});return;
  }
  await persistChunks(doc.id,doc.matter_id,extraction.chunks,extraction.method,extraction.pageCount,serverSha,job.id,{id:String(job.input?.requestedBy||"system-worker"),name:String(job.input?.requestedByName||"ContractTwin Worker")});
...
async function processOcr(job:ProcessingJob){
  ...
  await persistChunks(doc.id,doc.matter_id,result.chunks,"AZURE_DOCUMENT_INTELLIGENCE",result.pageCount,currentSha,extractionJobId,{id:String(job.input?.requestedBy||"system-worker"),name:String(job.input?.requestedByName||"ContractTwin Worker")});
...
async function processAnalysis(job:ProcessingJob){
  ...
  if(!runId){
    ...
    runId=await withTransaction(async client=>{
      const locked=await client.query("select id from processing_jobs where id=$1 and status='RUNNING' for update",[job.id]);
      if(!locked.rowCount)throw new TerminalJobError("Analysis job is no longer RUNNING.");
      ...
      await client.query("update processing_jobs set status='WAITING_EXTERNAL',output=$2::jsonb,external_operation_url=null,locked_by=null,locked_at=null,next_attempt_at=now() where id=$1",[job.id,JSON.stringify(output)]);
      return r.rows[0].id;
    });
    return;
  }
  ...
  if(state.next>=chunks.length){
    ...
    await withTransaction(async client=>{
      ...
      await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,JSON.stringify({analysisRunId:runId,findingCount:ready.length,rejected,modes:[...new Set(state.modes)],warnings:[...new Set(state.warnings)]})]);
  ...
  await continueJob(job.id,output,output.nextChunk>=chunks.length?0:1);
}
...
async function processTerms(job:ProcessingJob){
  ...
  if(!runId){
    ...
    runId=await withTransaction(async client=>{
      const locked=await client.query("select id from processing_jobs where id=$1 and status='RUNNING' for update",[job.id]);
      if(!locked.rowCount)throw new TerminalJobError("Term-extraction job is no longer RUNNING.");
  ...
  if(state.next>=chunks.length){
    ...
    await withTransaction(async client=>{
      ...
      const dependency=await client.query<{id:string}>(`insert into processing_jobs(matter_id,job_type,idempotency_key,created_by,input) values($1,'DEPENDENCY',$2,$3,$4::jsonb) on conflict(idempotency_key) do update set idempotency_key=excluded.idempotency_key returning id`,[job.matter_id,`dependency:${job.matter_id}:${runId}`,String(job.input?.requestedBy||"system-worker"),JSON.stringify({...job.input,termAnalysisRunId:runId})]);
      await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,JSON.stringify({analysisRunId:runId,termCount:inserted,rejected,dependencyJobId:dependency.rows[0].id})]);
...
async function processDependencies(job:ProcessingJob){
  ...
  await withTransaction(async client=>{
    ...
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,JSON.stringify({dependencyCount:objectIds.length,objectIds,...candidateCounts,...engine,termAnalysisRunId:triggerRunId,sourceDocumentIds,sourceRunIds,inputHash})]);
...
async function processPrecedence(job:ProcessingJob){
  ...
  await withTransaction(async client=>{
    ...
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,JSON.stringify({relationCount:objectIds.length,objectIds,...candidateCounts,...engine,sourceDocumentIds,inputHash})]);
...
async function processSnapshot(job:ProcessingJob){
  ...
  await withTransaction(async client=>{
    ...
    const agreement=(await client.query<any>(`select id,matter_id,version_number,label,status,effective_date::text effective_date,created_by,created_at::text created_at from agreement_versions where id=$1 and matter_id=$2 and status in ('APPROVED','EXECUTED')`,[requestedAgreementVersionId,job.matter_id])).rows[0];
    ...
    if(!economics)throw new TerminalJobError("The snapshot-bound economics run is not a validated scenario for the selected agreement version.");
    const decisions=(await client.query<any>("select d.id,d.agreement_version_id,d.finding_id,d.decision_type,d.rationale,d.decision_status,d.required_approver_role,d.requested_by,d.decided_by,d.requested_at::text requested_at,d.decided_at::text decided_at,coalesce(json_agg(json_build_object('id',dc.id,'sequenceNumber',dc.sequence_number,'text',dc.condition_text,'status',dc.condition_status,'evidence',dc.evidence,'resolvedBy',dc.resolved_by,'resolvedAt',dc.resolved_at::text) order by dc.sequence_number) filter(where dc.id is not null),'[]'::json) conditions from decisions d left join decision_conditions dc on dc.decision_id=d.id where d.matter_id=$1 and d.agreement_version_id=$2 group by d.id order by d.requested_at,d.id",[job.matter_id,agreement.id])).rows;
    ...
    const nextSteps:string[]=[];if(decisions.some((d:any)=>d.decision_status==="PENDING"))nextSteps.push("Resolve pending executive decisions under the recorded authority level.");if(agreement.status==="APPROVED")nextSteps.push("Complete authorized execution and freeze the executed agreement version.");else nextSteps.push("Retain the executed source set and monitor obligations against this frozen state.");
    ...
    const snapshotPresentation={topRisks,quantifiedExposure:economics,dependencies,negotiationActions,executiveDecisions:decisions,nextSteps};
    ...
    const result=await client.query<{id:string}>(`insert into executive_snapshots(matter_id,agreement_version_id,processing_job_id,snapshot_version,matter_context,source_manifest,source_manifest_canonical,top_risks,quantified_exposure,dependencies,negotiation_actions,executive_decisions,next_steps,source_state_hash,generated_by) values($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7,$8::jsonb,$9::jsonb,$10::jsonb,$11::jsonb,$12::jsonb,$13::jsonb,$14,$15) returning id`,[job.matter_id,agreement.id,job.id,latest+1,JSON.stringify(matterContext),sourceManifestCanonical,sourceManifestCanonical,JSON.stringify(topRisks),JSON.stringify(economics),JSON.stringify(dependencies),JSON.stringify(negotiationActions),JSON.stringify(decisions),JSON.stringify(nextSteps),sourceStateHash,requestedBy]);
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now(),locked_by=null,locked_at=null where id=$1",[job.id,JSON.stringify({snapshotId:result.rows[0].id,snapshotVersion:latest+1,agreementVersionId:agreement.id,economicsRunId:economics.id,requesterId:requestedBy,sourceAuditId:requestedAuditId,sourceStateHash,relianceEvidenceHash:requestedRelianceHash})]);
...
export async function processJob(job:ProcessingJob){
  const leaseClient=await getPool().connect();
  let lease=false;
  try{
    lease=(await leaseClient.query<{locked:boolean}>("select pg_try_advisory_lock(hashtextextended($1,0)) locked",[`processing-job:${job.id}`])).rows[0]?.locked===true;
    if(!lease)return;
    try{
      switch(job.job_type){
      ...
      if(analysisRunId&&!willRetry){
        ...
        await withTransaction(async client=>{
          await client.query("update processing_jobs set status='FAILED',error_message=$2,locked_by=null,locked_at=null,finished_at=now() where id=$1",[job.id,publicFailure]);
      ...
      }else publicFailure=(await failJob(job,error,retryable)).message;
      throw new Error(publicFailure);
    }
  }finally{
    try{if(lease)await leaseClient.query("select pg_advisory_unlock(hashtextextended($1,0))",[`processing-job:${job.id}`]);}
    finally{leaseClient.release();}
  }
}
