...
export type RuntimeDatabasePrincipalRow = QueryResultRow & {
  session_user_name: string;
  current_user_name: string;
  identities_match: boolean;
  role_attributes_safe: boolean;
  role_membership_safe: boolean;
  inbound_role_membership_safe: boolean;
  owner_membership_safe: boolean;
  dangerous_membership_safe: boolean;
  database_create_safe: boolean;
  database_temp_safe: boolean;
  schema_create_safe: boolean;
  application_function_execute_safe: boolean;
  approved_runtime_functions_ready: boolean;
  application_table_dml_ready: boolean;
  application_sequence_privileges_safe: boolean;
  table_trigger_safe: boolean;
  table_truncate_safe: boolean;
  table_references_safe: boolean;
  table_maintain_safe: boolean;
  replication_mode_safe: boolean;
  replication_parameter_safe: boolean;
  migration_receipts_read_only: boolean;
  release_control_tables_read_only: boolean;
};
...
const REQUIRED_SAFE_FIELDS = [
  "identities_match",
  "role_attributes_safe",
  "role_membership_safe",
  "inbound_role_membership_safe",
  "owner_membership_safe",
  "dangerous_membership_safe",
  "database_create_safe",
  "database_temp_safe",
  "schema_create_safe",
  "application_function_execute_safe",
  "approved_runtime_functions_ready",
  ...
  "table_references_safe",
  "table_maintain_safe",
  "replication_mode_safe",
  "replication_parameter_safe",
  "migration_receipts_read_only",
  "release_control_tables_read_only"
] as const;

export function evaluateRuntimeDatabasePrincipal(row:RuntimeDatabasePrincipalRow|undefined){
  const errors:string[]=[];
  if(!row)errors.push("runtime database principal evidence is missing");
  else for(const field of REQUIRED_SAFE_FIELDS){if(row[field]!==true)errors.push(`runtime database principal failed ${field}`);}
  return {ok:errors.length===0,errors,checkedCount:REQUIRED_SAFE_FIELDS.length};
}
