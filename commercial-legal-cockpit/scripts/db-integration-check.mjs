...
async function createGraphJob({matterId,agreementVersionId,scopeType,documentIds,runIds,inputHash,objectIds,rawCount,rejectedCount}){
  ...
  const job=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,$2,'RUNNING',$3,$4::jsonb,'ci-worker',now()) returning id`,[matterId,jobType,`ci-${scopeType.toLowerCase()}-${randomUUID()}`,JSON.stringify(input)])).rows[0];
  ...
  await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,finished_at=now() where id=$1",[job.id,JSON.stringify(output)]);
  return job;
}
...
try{
  ...
  try{
    ...
    await client.query("update processing_jobs set status='RUNNING',started_at=now() where id=$1",[extractJob.id]);
    ...
    await client.query(`update processing_jobs set status='SUCCEEDED',output='{"chunkCount":1}'::jsonb,finished_at=now() where id=$1`,[extractJob.id]);
    ...
    const decision=(await client.query(`insert into decisions(matter_id,agreement_version_id,decision_type,rationale,requested_by,required_approver_role) values($1,$2,'ESCALATE','Synthetic decision rationale','ci-requester','APPROVER') returning id`,[matter.id,version.id])).rows[0];
    ...
    await expectFailure("sp_self_decision",()=>client.query("update decisions set decision_status='APPROVED',decided_by='ci-requester',decided_at=now() where id=$1",[decision.id]),error=>String(error.message).toLowerCase().includes("independent"));
    await client.query("update decisions set decision_status='APPROVED',decided_by='ci-approver',decided_at=now() where id=$1",[decision.id]);
    ...
    const waiverDecision=(await client.query(`insert into decisions(matter_id,agreement_version_id,decision_type,rationale,requested_by,required_approver_role) values($1,$2,'ESCALATE','Waiver authority test','ci-requester-4','ADMIN') returning id`,[matter.id,version.id])).rows[0];
    ...
    await client.query("update decisions set decision_status='APPROVED',decided_by='ci-admin',decided_at=now() where id=$1",[waiverDecision.id]);
    ...
    const badCandidateJob=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,'DEPENDENCY','RUNNING',$2,$3::jsonb,'ci-worker',now()) returning id`,[matter.id,`ci-bad-candidates-${randomUUID()}`,JSON.stringify({agreementVersionId:version.id,sourceDocumentIds:[doc.id],sourceTermAnalysisRunIds:[termRun.id],graphVersion:GRAPH_VERSION})])).rows[0];
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,finished_at=now() where id=$1",[badCandidateJob.id,JSON.stringify({dependencyCount:0,objectIds:[],rawCandidateCount:1,rejectedCandidateCount:1,modelName:MODEL,promptVersion:DEPENDENCY_PROMPT,schemaVersion:DEPENDENCY_SCHEMA,sourceDocumentIds:[doc.id],sourceRunIds:[termRun.id],inputHash:depHash})]);
    ...
    await expectFailure("sp_snapshot_hash",()=>client.query(`insert into executive_snapshots(matter_id,snapshot_version,matter_context,source_manifest,source_manifest_canonical,source_state_hash,generated_by) values($1,1,'{}'::jsonb,$2::jsonb,$3,$4,'ci-user')`,[matter.id,canonical,canonical,"d".repeat(64)]),error=>String(error.message).toLowerCase().includes("hash"));
    await expectFailure("sp_snapshot_without_receipt",()=>client.query(`insert into executive_snapshots(matter_id,snapshot_version,matter_context,source_manifest,source_manifest_canonical,source_state_hash,generated_by) values($1,1,'{}'::jsonb,$2::jsonb,$3,$4,'ci-user')`,[matter.id,canonical,canonical,snapshotHash]),error=>String(error.message).includes("EXECUTIVE_SUMMARY processing-job receipt"));
    ...
    await expectFailure("sp_execution_without_validated_economics",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[noEconomicsVersion.id]),error=>String(error.message).toLowerCase().includes("validated economics run"));
    ...
    await expectFailure("sp_execution_dirty_source",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).toLowerCase().includes("clean, extracted, hash-verified, and active"));
    await client.query("update documents set extraction_status='EXTRACTED' where id=$1",[doc.id]);

    const agreementEvidence=(await client.query("select id,matter_id,version_number,label,status,effective_date::text effective_date,created_by,created_at::text created_at from agreement_versions where id=$1",[version.id])).rows[0];
    const economicsEvidence=(await client.query("select id,matter_id,agreement_version_id,inputs,outputs,formula_version,review_status,reviewed_by,reviewed_at::text reviewed_at,review_note,created_by,created_at::text created_at from economics_runs where id=$1",[economics.id])).rows[0];
    ...
    const relianceEvidence={legalRelianceEnabled:true,legalRelianceReady:true,enginePoliciesReady:true,validation:{id:randomUUID()}};
    ...
    const createSnapshotReceipt=async(snapshotVersion,{badTerminal=false,badMatter=false}={})=>{
      const snapshotJob=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,'EXECUTIVE_SUMMARY','RUNNING',$2,$3::jsonb,'ci-admin',now()) returning id`,[matter.id,`ci-snapshot-${randomUUID()}`,JSON.stringify({requestedBy:"ci-admin",requestedAgreementVersionId:version.id,requestedEconomicsRunId:economics.id,requestedAuditId:sourceAuditId,requestedRelianceEvidence:relianceEvidence,requestedRelianceHash:relianceEvidenceHash})])).rows[0];
      const publicationReceipt={jobId:snapshotJob.id,requesterId:"ci-admin",agreementVersionId:version.id,economicsRunId:economics.id,sourceAuditId,relianceEvidenceHash};
      ...
      const snapshot=(await client.query(`insert into executive_snapshots(matter_id,agreement_version_id,processing_job_id,snapshot_version,matter_context,source_manifest,source_manifest_canonical,top_risks,quantified_exposure,dependencies,negotiation_actions,executive_decisions,next_steps,source_state_hash,generated_by) values($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7,($6::jsonb)#>'{snapshotPresentation,topRisks}',($6::jsonb)#>'{snapshotPresentation,quantifiedExposure}',($6::jsonb)#>'{snapshotPresentation,dependencies}',($6::jsonb)#>'{snapshotPresentation,negotiationActions}',($6::jsonb)#>'{snapshotPresentation,executiveDecisions}',($6::jsonb)#>'{snapshotPresentation,nextSteps}',$8,'ci-admin') returning id`,[matter.id,version.id,snapshotJob.id,snapshotVersion,JSON.stringify(frozenMatterContext),manifestCanonical,manifestCanonical,sourceStateHash])).rows[0];
      const terminalOutput={snapshotId:snapshot.id,snapshotVersion,agreementVersionId:version.id,economicsRunId:economics.id,requesterId:"ci-admin",sourceAuditId,sourceStateHash,relianceEvidenceHash};
      if(badTerminal)delete terminalOutput.sourceAuditId;
      return {snapshot,snapshotJob,terminalOutput};
    };
    await expectFailure("sp_snapshot_forged_matter_context",()=>createSnapshotReceipt(1,{badMatter:true}),error=>String(error.message).includes("exact locked matter and customer state"));
    const validSnapshotReceipt=await createSnapshotReceipt(1);
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now() where id=$1",[validSnapshotReceipt.snapshotJob.id,JSON.stringify(validSnapshotReceipt.terminalOutput)]);
    ...
    if(verifiedSnapshotReceipt.verified!==true)throw new Error("A complete bound executive-snapshot receipt did not verify on read.");
    ...
    const badTerminalReceipt=await createSnapshotReceipt(2,{badTerminal:true});
    await expectFailure("sp_snapshot_terminal_receipt",()=>client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,error_message=null,finished_at=now() where id=$1",[badTerminalReceipt.snapshotJob.id,JSON.stringify(badTerminalReceipt.terminalOutput)]),error=>String(error.message).includes("terminal output does not bind"));
    ...
    await client.query("update decisions set decision_status='REJECTED',decided_by='ci-approver',decided_at=now() where id=$1",[rejectedAcceptance.id]);
    await expectFailure("sp_execution_rejected_acceptance",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).toLowerCase().includes("approved affirmative decision"));
    const approvedAcceptance=(await client.query(`insert into decisions(matter_id,agreement_version_id,finding_id,decision_type,rationale,requested_by,required_approver_role) values($1,$2,$3,'ACCEPT','Test affirmative version authority','ci-requester-2','APPROVER') returning id`,[matter.id,version.id,approvalFinding.id])).rows[0];
    ...
    await client.query("update decisions set decision_status='APPROVED',decided_by='ci-approver',decided_at=now() where id=$1",[approvedAcceptance.id]);
    await expectFailure("sp_execution_pending_condition",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).toLowerCase().includes("pending approved-decision conditions"));
    ...
    await expectFailure("sp_rejected_grounding_execution",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).includes("current clause-risk and term-extraction"));
    ...
    await expectFailure("sp_fallback_model_execution",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).includes("current clause-risk and term-extraction"));
    const currentRiskRun=(await client.query(`insert into analysis_runs(matter_id,document_id,run_type,status,model_name,prompt_version,schema_version,input_sha256,source_chunk_count,output_count,rejected_ungrounded_count,created_by,started_at,finished_at) values($1,$2,'CLAUSE_RISK','SUCCEEDED',$3,$4,$5,$6,1,0,0,'ci-worker','2100-01-01 00:00:00+00','2100-01-01 00:00:01+00') returning id`,[matter.id,doc.id,MODEL,CLAUSE_PROMPT,CLAUSE_SCHEMA,sourceInputHash])).rows[0];
    await attest({matterId:matter.id,scopeType:"CLAUSE_RISK",scopeId:currentRiskRun.id});
    await expectFailure("sp_wrong_version_graph_receipts",()=>client.query("update agreement_versions set status='EXECUTED' where id=$1",[version.id]),error=>String(error.message).includes("exact current version term runs"));

    const dependencyJob1=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,'DEPENDENCY','RUNNING',$2,$3::jsonb,'ci-worker',now()) returning id`,[matter.id,`ci-dependency-reviewed-reject-${randomUUID()}`,JSON.stringify({agreementVersionId:version.id,sourceDocumentIds:[doc.id],sourceTermAnalysisRunIds:[termRun.id],graphVersion:GRAPH_VERSION,termAnalysisRunId:termRun.id})])).rows[0];
    ...
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,finished_at=now() where id=$1",[dependencyJob1.id,JSON.stringify({dependencyCount:1,objectIds:[rejectedDependency.id],rawCandidateCount:1,rejectedCandidateCount:0,modelName:MODEL,promptVersion:DEPENDENCY_PROMPT,schemaVersion:DEPENDENCY_SCHEMA,termAnalysisRunId:termRun.id,sourceDocumentIds:[doc.id],sourceRunIds:[termRun.id],inputHash:depHash})]);
    await attest({matterId:matter.id,scopeType:"DEPENDENCY",scopeId:dependencyJob1.id});

    const dependencyJob2=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,'DEPENDENCY','RUNNING',$2,$3::jsonb,'ci-worker',now()) returning id`,[matter.id,`ci-dependency-corrected-${randomUUID()}`,JSON.stringify({agreementVersionId:version.id,sourceDocumentIds:[doc.id],sourceTermAnalysisRunIds:[termRun.id],graphVersion:GRAPH_VERSION,termAnalysisRunId:termRun.id})])).rows[0];
    ...
    await client.query("update processing_jobs set status='SUCCEEDED',output=$2::jsonb,finished_at=now() where id=$1",[dependencyJob2.id,JSON.stringify({dependencyCount:1,objectIds:[correctedDependency.id],rawCandidateCount:1,rejectedCandidateCount:0,modelName:MODEL,promptVersion:DEPENDENCY_PROMPT,schemaVersion:DEPENDENCY_SCHEMA,termAnalysisRunId:termRun.id,sourceDocumentIds:[doc.id],sourceRunIds:[termRun.id],inputHash:depHash})]);
    await attest({matterId:matter.id,scopeType:"DEPENDENCY",scopeId:dependencyJob2.id});

    const maskedJob=(await client.query(`insert into processing_jobs(matter_id,job_type,status,idempotency_key,input,created_by,started_at) values($1,'DEPENDENCY','RUNNING',$2,$3::jsonb,'ci-worker',now()) returning id`,[matter.id,`ci-dependency-masked-${randomUUID()}`,JSON.stringify({agreementVersionId:version.id,sourceDocumentIds:[doc.id],sourceTermAnalysisRunIds:[termRun.id],graphVersion:GRAPH_VERSION,termAnalysisRunId:termRun.id})])).rows[0];
  ...
  console.log("Database integration controls passed: source/extraction lineage, run-only publication, microsecond review attestations, version-bound current-engine graph receipts, rejection-free zero-output handling, structured decision conditions/Admin waivers, current economics, execution freeze/serialization, terminal validation manifests, approved-successor uniqueness, legal holds, immutable evidence, and race-safe run-scoped graph idempotency.");
