import { AccessError, accessErrorResponse, requireMatterAccess } from "@/lib/access";
...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
  try{
    ...
    const result=await withTransaction(async client=>{const counselAuthority=await client.query("select 1 from app_user_capabilities where user_id=$1 and capability='LEGAL_COUNSEL_ATTEST' and active=true for share",[principal.userId]);if(counselAuthority.rowCount!==1)throw new AccessError("Active legal-counsel attestation authority is required to record a validated relation.",403);const inserted=await client.query<{id:string}>(`insert into document_relations(matter_id,origin,source_document_id,target_document_id,relation_type,source_locator,rationale,confidence,review_status,created_by,reviewed_by,reviewed_at,review_note) values($1,'COUNSEL',$2,$3,$4,$5,$6,1,'VALIDATED',$7,$7,now(),$6) returning id`,[matterId,body.sourceDocumentId,body.targetDocumentId,body.relationType,body.sourceLocator?.trim()||null,rationale,principal.userId]);await client.query(`insert into audit_events(actor_user_id,actor_name,action,matter_id,entity_type,entity_id,metadata) values($1,$2,'DOCUMENT_RELATION_RECORDED',$3,'document_relation',$4,$5::jsonb)`,[principal.userId,principal.name,matterId,inserted.rows[0].id,JSON.stringify({sourceDocumentId:body.sourceDocumentId,targetDocumentId:body.targetDocumentId,relationType:body.relationType,origin:"COUNSEL"})]);return inserted;});
