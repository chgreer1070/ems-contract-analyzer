...
export async function POST(request:Request){
  ...
  }catch(error){const access=accessErrorResponse(error);if(access)return access;return Response.json({ok:false,error:error instanceof Error?error.message:"Unable to start legal validation."},{status:500});}
}
