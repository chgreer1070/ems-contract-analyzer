...
type PurgeRow={
  ...
  matter_hold:boolean;matter_retention:string|null;retention_end:string|null;retention_expired:boolean;
};
...
async function lockPurgeState(client:PoolClient,id:string):Promise<PurgeRow>{
  ...
  const row=await client.query<PurgeRow>(`select pr.id,pr.matter_id,pr.document_id,pr.requested_by,pr.status,pr.approved_by,d.blob_pathname,d.filename,d.legal_hold document_hold,d.retention_until::text document_retention,d.deletion_status,m.legal_hold matter_hold,m.retention_until::text matter_retention,greatest(d.retention_until,m.retention_until)::text retention_end,(greatest(d.retention_until,m.retention_until) is not null and current_date>greatest(d.retention_until,m.retention_until)) retention_expired from purge_requests pr join documents d on d.id=pr.document_id join matters m on m.id=pr.matter_id where pr.id=$1 for update of pr`,[id]);
  if(!row.rows[0])throw new PurgeActionError("Purge request not found.",404);
  return row.rows[0];
}

async function assertActiveAdmin(client:PoolClient,userId:string){
  const role=(await client.query<{role:string}>("select role from app_user_roles where user_id=$1 and active=true for share",[userId])).rows[0]?.role;
  if(role!=="ADMIN")throw new PurgeActionError("Active Admin authority is required at purge action time.",403);
}
...
function assertExecutable(row:PurgeRow){
  ...
  if(!row.retention_end)throw new PurgeActionError("No retention end date is recorded; purge execution is blocked.",409);
  if(row.retention_expired!==true)throw new PurgeActionError("The applicable retention period has not expired according to the database clock.",409);
  return row.retention_end;
}
...
export async function PATCH(request:Request,context:{params:Promise<{id:string}>}){
  try{
    ...
    if(body.action==="APPROVE"){
      await withTransaction(async client=>{await assertActiveAdmin(client,principal.userId);const row=await lockPurgeState(client,id);if(row.status!=="PENDING")throw new PurgeActionError("Only PENDING requests can be approved.",409);if(row.requested_by===principal.userId)throw new PurgeActionError("The requester cannot approve their own purge request.",409);if(row.document_hold||row.matter_hold)throw new PurgeActionError("Purge approval is blocked by an active legal hold.",409);await client.query("update purge_requests set status='APPROVED',approved_by=$2,approved_at=now() where id=$1",[id,principal.userId]);await client.query(`insert into audit_events(actor_user_id,actor_name,action,matter_id,entity_type,entity_id,metadata) values($1,$2,'PURGE_APPROVED',$3,'purge_request',$4,$5::jsonb)`,[principal.userId,principal.name,row.matter_id,id,JSON.stringify({documentId:row.document_id})]);});
    ...
    const prepared=await withTransaction(async client=>{await assertActiveAdmin(client,principal.userId);const row=await lockPurgeState(client,id);const retentionEnd=assertExecutable(row);if(row.deletion_status==="ACTIVE"){await client.query("update documents set deletion_status='PENDING_PURGE' where id=$1",[row.document_id]);await client.query(`insert into audit_events(actor_user_id,actor_name,action,matter_id,entity_type,entity_id,metadata) values($1,$2,'PURGE_EXECUTION_STARTED',$3,'purge_request',$4,$5::jsonb)`,[principal.userId,principal.name,row.matter_id,id,JSON.stringify({documentId:row.document_id,retentionEnd,authorityRole:"ADMIN",databaseClockEvaluated:true})]);}return {...row,retentionEnd};});

    await withTransaction(async client=>{
      await assertActiveAdmin(client,principal.userId);const row=await lockPurgeState(client,id);const retentionEnd=assertExecutable(row);
      try{
