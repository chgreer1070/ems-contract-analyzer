...
function parsePayload(raw: string | null | undefined) {
  ...
  const parsed = JSON.parse(raw) as {
    matterId?: string; documentType?: string; versionLabel?: string; filename?: string;
    sha256?: string; sizeBytes?: number; sourceStatus?: string;
  };
...
export async function POST(request: Request) {
  try {
    if (!process.env.BLOB_READ_WRITE_TOKEN) {
      return Response.json({ ok: false, error: "Private document storage is not configured." }, { status: 503 });
    }
    if (!databaseConfigured()) {
      return Response.json({ ok: false, error: "Document registration requires DATABASE_URL." }, { status: 503 });
    }
    ...
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        if (!pathname.startsWith(`contracts/${metadata.matterId}/`)) {
          throw new Error("Upload pathname does not match the authorized matter.");
        }
        ...
        return {
          ...
          tokenPayload: JSON.stringify({ ...metadata, principal })
        };
      },
      onUploadCompleted: async ({ blob, tokenPayload }) => {
        ...
        if (!principal || principal.demo) throw new Error("Invalid upload principal.");

        const inserted = await query<{ id: string }>(
          `insert into documents
            (matter_id, filename, document_type, version_label, mime_type, size_bytes, blob_url, blob_pathname,
             sha256, integrity_status, source_status, uploaded_by)
           values ($1,$2,$3,$4,$5,$6,$7,$8,$9,'CLIENT_HASHED',$10,$11)
           returning id`,
          [
            payload.matterId,
            payload.filename,
