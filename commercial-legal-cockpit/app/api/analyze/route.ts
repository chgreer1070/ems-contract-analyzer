...
export async function POST(request: Request) {
  try {
    ...
    const findingIds = await persistFindings({principal,matterId:body.matterId,documentId:body.documentId,findings,modelName:analysis.modelName,promptVersion:PROMPT_VERSION});

    await writeAuditEvent({principal,action:"ANALYSIS_RUN",matterId:body.matterId,entityType:"analysis",metadata:{mode:analysis.mode,modelName:analysis.modelName,promptVersion:PROMPT_VERSION,findingCount:findings.length,rejectedUngroundedFindings:analysis.rejectedUngroundedFindings,legalRelianceEnabled}});

    return Response.json({ok:true,mode:analysis.mode,humanReviewRequired:true,legalRelianceEnabled,promptVersion:PROMPT_VERSION,findings,findingIds,rejectedUngroundedFindings:analysis.rejectedUngroundedFindings,warning:analysis.warning});
  } catch (error) {
