...
export async function POST(request:Request,context:{params:Promise<{id:string}>}){
  try{
    ...
    await writeAuditEvent({principal,action:"AGREEMENT_VERSION_CREATED",matterId:row.rows[0].matter_id,entityType:"agreement_version_status",entityId:id,metadata:{from:row.rows[0].status,to:body.status}});
